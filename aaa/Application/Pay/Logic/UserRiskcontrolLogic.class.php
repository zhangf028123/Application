...
class UserRiskcontrolLogic extends RiskcontrolLogic
{
    ...
    public function __construct($pay_amount = '0.00', $user_id = 0)
    {
        ...
        if ($this->config_info) {
            ...
            $this->config_info['unit_frist_paying_time'] = $this->member_info['unit_frist_paying_time'];    // 单位时间已交易的第一笔时间
            $this->config_info['unit_paying_number']     = $this->member_info['unit_paying_number'];        // 单位时间已交易次数
            $this->config_info['unit_paying_amount']     = $this->member_info['unit_paying_amount'];        // 单位时间已交易金额
            $this->config_info['last_paying_time']       = $this->member_info['last_paying_time'];          // 当天最后一笔已交易时间
            $this->config_info['paying_money']           = $this->member_info['paying_money'];              // 当天已交易金额
        }
    }
    ...
    public function monitoringData()
    {
        if ($this->config_info) {
            ...
            $the_total_volume_judge = $this->theTotalVolume(function () {
                ...
                Log::record("m_Member 1 新一天交易量清0：id={$this->member_info['id']},res=$res, error={$this->m_Member->getError()},dberror={$this->m_Member->getDbError()} ",'ERR',true);
            });
            ...
            $unit_timeoperate_judge = $this->unitTimeOperate(function () {
                ...
                Log::record("m_Member 2 新一天交易量清0：id={$this->member_info['id']},\$reuslt=$reuslt, error={$this->m_Member->getError()},dberror={$this->m_Member->getDbError()} ",'ERR',true);
            });
