...
class PhoneBillController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        Log::record('PhoneBill pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
       ...
       if ($response['status'] != '1' || $contentType == 'json'){
           if($response['status'] != '1'){    // 记录下单失败的记录
               ...
               file_put_contents("Data/phonebill_failed.txt",json_encode($response).",gateway=".$return['gateway'].",storeid=  ".$data['storeid']."\n", FILE_APPEND);
           } else {
               $response['status'] = 'ok';
           }
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，phone bill notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($sign == $response['sign'] && $response['is_pay'] == 'true') {
            try{
                ...
                if(!$o){
                    Log::record('小额话费回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("小额话费回调失败,金额不等：{$response['money'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $old_order = $Order->where(['upstream_order'=>$response['order_no']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["out_order_id"]){
                    Log::record("小额话费回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
            ...
            //未支付。。。
            Log::record('小额话费订单失败：'.json_encode($response),'ERR',true);
            exit("error:not pay");
        }
        else {
