...
class AliCodeBlController extends PayController
{
    ...
    {
        $response  = $_REQUEST;
        Log::record('支付宝个码（bl）订单失败：'.$response.'--'.json_decode($response).'--'.json_encode($response),'ERR',true);

        $clientip = $_SERVER['REMOTE_ADDR'];
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，AliCodeBl notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($sign == $response['sign'] && $response['status'] == 'success') {
            try{
                ...
                if(!$o){
                    Log::record('支付宝个码（bl）回调失败,找不到订单：'.$response,'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("支付宝个码（bl）回调失败,金额不等：{$response['amount'] } != {$pay_amount},".$response,'ERR',true);
                    exit('error: amount error!');
                }

                $old_order = $Order->where(['upstream_order'=>$response['pt_order']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["sh_order"]){
                    Log::record("支付宝个码（bl）回调失败,重复流水号  ：".$response.'旧订单号'.$old_order['order_id'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('支付宝个码（bl）回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } elseif ($sign == $response['sign']) {
            //未支付。。。
            Log::record('支付宝个码（bl）订单失败：'.$response,'ERR',true);
            exit("error:not pay");
        }
        else {
