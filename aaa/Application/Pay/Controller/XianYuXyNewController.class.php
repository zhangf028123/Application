...
class XianYuXyNewController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('XianYuXyNew pay url=' . $return['gateway'] . ',data=' . json_encode($data) . ',response=' . $response, 'ERR', true);
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，XianYuXyNew  notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
