...
class XianYuXySdkNewController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('XianYuXySdkNew pay url=' . $return['gateway'] . ',data=' . json_encode($data) . ',response=' . $response, 'ERR', true);
    ...
    {
        $response  = $_REQUEST;
                Log::record(" XianYuXySdknew \$response=".json_encode($response),'ERR',true);

        $clientip = $_SERVER['REMOTE_ADDR'];
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，XianYuXySdkNew  notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        //Log::record('ssssss:'.$sign.'---'.$response['sign'], 'ERR', true);
        //Log::record('ssssss:'.$publiKey.'---'.json_encode($data), 'ERR', true);
