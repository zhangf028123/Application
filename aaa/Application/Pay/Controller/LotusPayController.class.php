...
class LotusPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            'pass_code' => $return['appid'],//是	pay.alipay.wappay 支付方式
            'subject' => '测试',
            'body' => '装修材料',
        ...
        Log::record('LotusPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('LotusPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '0' ) {
            ...
            Log::record('LotusPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
...
//        $response = $_REQUEST;
        ...
        $clientip = $_SERVER['REMOTE_ADDR'];
        Log::record(" LotusPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，LotusPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $response = json_decode($response,true);
        $data = [
            ...
            'original_trade_no' => $response['original_trade_no'],//原始订单号：建议保存，可作为异常凭证
            'money' =>$response['money'],
            'notify_time' => $response['notify_time'],//订单时间
            'status' => $response['status'],//币种类
            'sign' => $response['sign'],
        ];

//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["status"])||$response["status"] != '2' ){
            Log::record(' LotusPay回调 orderid= ' . $response['out_trade_no'] . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' LotusPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
