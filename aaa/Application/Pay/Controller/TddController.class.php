...
class TddController extends PayController{
  ...
  public function Pay($array){
    ...
    $notifyurl   = $this->_site . 'Pay_Tdd_notifyurl.html'; //异步通知
    ...
    header("location: {$url}");*/
      // $response = file_get_contents($url);   // 这会爆500错误，
    ...
    $contentType = I("request.content_type");
      ...
      if ($response['code'] != '1' || $contentType == 'json'
          ...
          if($response['code'] != '1'){    // 记录下单失败的记录
              file_put_contents("Data/tdd_failed.txt",json_encode($response).",gateway={$return['gateway']},\$_REQUEST=".json_encode($_REQUEST).",\$data=".json_encode($data)."\n", FILE_APPEND);
          }
  ...
  public function notifyurl()
  {
    ...
    $ip = getIP(); // 可能是伪造的
    Log::record("tdd notifyurl clientip=".json_encode($clientip)."\$_POST=".json_encode($_POST),'INFO',true);
    if($clientip != $ip){
      Log::record("伪造的ip，tdd notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
      die("not ok1");
    }
    ...
    if ($result) {
      ...
      try{
        ...
        if(!$o){
          Log::record('淘多多回调失败,找不到订单：'.json_encode($response),'ERR',true);
        ...
        if($diff < -0.5 || $diff > 1 ){
          Log::record("淘多多回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
          exit('error: fuck1!');
        }
        ...
        Log::record('淘多多回调失败,发生异常：'.$e->getMessage(),'ERR',true);
        exit("Exception");
      }
      //}
    } else {
    ...
    private function _verify_md5($data, $apiKey){
      $str = "appId={$data['appId']}&systemOrderBn={$data['systemOrderBn']}&sOrderBn={$data['sOrderBn']}&amount={$data['amount']}&".$apiKey;
        ...
        if($calcSign != $strData){
            Log::record("淘多多回调失败,MD5不对：$calcSign != $strData, str=$str".json_encode($data),'ERR',true);
            return false;
        }
        return true;
    }
