...
class AiHuiShouPayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('AiHuiShouPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('AiHuiShouPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1') {
            ...
            Log::record('AiHuiShouPay  orderid  = ' . $return['orderid'] . '.pay failt ', 'ERR', true);
...
//        $response = $_REQUEST;
        ...
        Log::record(" AiHuiShouPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，AiHuiShouPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $response = json_decode($response, true);
        $data = [
            ...
            "mch_id" => $response["mch_id"], // 订单状态,2表示支付成功,其它都为失败
            "name" => $response["name"], //
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if ($response["status"] == '1') {
            ...
            if ($result) {
                ...
                Log::record(' AiHuiShouPay回调 orderid= ' . $response['out_trade_no'] . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }

        } else {
            Log::record('AiHuiShouPay回调 orderid= ' . $response['mchOrderNo'] . '  状态失败 $response' . json_encode($response), 'ERR', true);
            exit("fail");
        }
