...
class XinGuangPayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('XinGuangPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('XinGuangPay  $response is empty ', 'ERR', true);
            exit("data empyt");
        }
        if ($response['status'] == 'success' ) {
            ...
            Log::record('XinGuangPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
...
//        $response = $_REQUEST;
        ...
        Log::record(" XinGuangPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，XinGuangPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $response = json_decode($response,true);
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["status"])||$response["status"] != 'success' ){
            Log::record(' XinGuangPay回调 orderid= ' . $response['orderid'] . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' XinGuangPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
