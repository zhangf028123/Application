...
class XianYuTsController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        Log::record('XianYuTs pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，XianYuTs notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
