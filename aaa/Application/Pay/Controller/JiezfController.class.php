...
class JiezfController extends PayController
{
    public function Pay($array){
        ...
        Log::record('jiezf pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" jiezf \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，jiezf notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result) {
            ...
            Log::record('jiezf error:check sign Fail!','ERR',true);
