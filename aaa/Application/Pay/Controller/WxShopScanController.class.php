...
class WxShopScanController extends PayController
{
    ...
    public function Pay($array)
    {
     ...
     //       'channel_id' => $return['appid'],
        ...
        Log::record('WxShopScan pay url=' . $return['gateway'] . 'data=' . json_encode($data) . 'response=' . $response . "cost time={$cost_time}ms", 'ERR', true);
        ...
        if ($response['code'] != '0' || $contentType == 'json'){
            if($response['code'] != '0'){    // 记录下单失败的记录
                ...
                file_put_contents("Data/WxShopscan_failed.txt",json_encode($response).",gateway=".$return['gateway'].",storeid=  ".$data['storeid']."\n", FILE_APPEND);
            } else {
                $response['status'] = 'ok';
            }
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，WxShopscan notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($sign == $response['sign'] && $response['orderStatus'] == 'paid') {
            try{
                ...
                if(!$o){
                    Log::record('微信商户扫码回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("微信商户扫码回调失败,金额不等：{$response['txnAmt'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $diff1 = $response['paidAmt'] - $pay_amount;
                if($diff1 <= -1 || $diff1 >= 1 ){ // 允许误差一块钱
                    Log::record("微信商户扫码回调失败,金额不等：{$response['paidAmt'] } != {$pay_amount},".json_encode($response),'ERR',true);
                ...
                if( $old_order && $old_order['pay_orderid'] != $response["mercOrderId"]){
                    Log::record("微信商户扫码回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('微微信商户扫码失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } elseif ($sign == $response['sign']) {
            //支付失败。。。
            Log::record('微信商户扫码订单失败：'.json_encode($response),'ERR',true);
            exit("error: pay fail");
        }
        else {
