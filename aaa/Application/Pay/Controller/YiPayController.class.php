...
class YiPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            "appId" =>"c93a7582bb6c41f59ece948c4cde0c29",  //商户应用ID  ????
        ...
        Log::record('YiPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('YiPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['retCode'] == 'SUCCESS' ) {
            header("location: {$response['payParams']['codeUrl']}");
            exit();
        } else {
            Log::record('YiPay  orderid  = ' . $return['orderid'] . '.pay failt ', 'ERR', true);
...
// 异步通知
    public function notifyurl()
    {
        $response = $_REQUEST;
        $clientip = $_SERVER['REMOTE_ADDR'];
        Log::record(" YiPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，YiPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if (!is_null($response["status"]) &&($response["status"] == '2'||$response["status"] == '3')) {
            ...
            if ($result) {
                ...
                $paymoney = $_REQUEST["amount"]/100;// 交易金额
                try {
                    $Order = M("Order");
                ...
                Log::record(' YiPay回调 orderid= ' . $response['out_trade_no'] . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }

        } else {
            Log::record('YiPay回调 orderid= ' . $response['mchOrderNo'] . '  状态失败 $response' . json_encode($response), 'ERR', true);
            exit("fail");
        }
