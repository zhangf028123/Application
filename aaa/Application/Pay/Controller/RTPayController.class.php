...
class RTPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            'merchantOrderNo' => $return['orderid'], //商户订单号
            'amount' => sprintf("%.2f", $return["amount"]),
            'channelNo'=>$return['appid'],
            'callbackAdd'=>$return['notifyurl'], //异步回调 服务器回调的通知地址
            'resultAdd' => $return['notifyurl']."notereuslt", //回调 服务器回调的通知地址  同步地址
            'extend'=>$this->getRand(),//随机字符串，英文或者字母组成10-50位
        ];
        ...
        Log::record('RTPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('RTPay  $response is empty ', 'ERR', true);
            exit("data empyt");
        }
        if ($response['code'] == '200' ) {
            ...
            Log::record('RTPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
...
//        $response = $_REQUEST;
        ...
        Log::record(" RTPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，RTPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $response = json_decode($response,true);
        $data = [
            'merchantOrderNo' => $response["merchantOrderNo"],//商户订单号
            'amount' => $response['amount'],//订单金额, 单位元.两位小数 例如: 1.00
            'payAmount' => $response["payAmount"],//单位元 两位小数 例如: 1.00
            'payStatus'=> $response["payStatus"],//支付状态 0=未支付 1=已支付 2=支付错误
            'payTime' => $response['payTime'],//支付时间 yyyy-MM-dd HH:mm:ss 格式
            'orderNo' => $response['orderNo'],//平台订单号
            'extend' => $response['extend'],//扩展参数, 下单时传入,原样返回. 如非空. 需要参与签名
            'sign' => $response['sign'],
        ];

//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["payStatus"])||$response["payStatus"] != '1' ){
            Log::record(' RTPay回调 orderid= ' . $response['orderid'] . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            Log::record('----------------- 上游回调 参与签名的 data= ' . json_encode($data), 'ERR', true);
            $orderid = $response["merchantOrderNo"];//自己的订单号
            $upstream_order_id = $response["orderNo"];// 上游的订单id,
            $paymoney = $response["payAmount"];// 交易金额
            ...
            if ($result) {
                ...
                Log::record(' RTPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    ...
    private function getSign($secret, $data)
    {
        ...
        Log::record('RTPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
