...
class WBZfbHfController extends PayController
{
    public function Pay($array){
        ...
        Log::record('WBZfbHf $parameter ='.json_encode($parameter),'ERR',true);
...
//            'pay_bankcode' => $return['appid'], // ,这个appid  对于什么东？
        ...
        Log::record('WBZfbHf pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    */
    ...
    public function notifyurl()
    {
        ...
        Log::record(" WBZfbHf \$response=".$response,'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，WBZfbHf notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if (makeSign($response, $publiKey) == $response['sign']) {
            ...
            Log::record('WBZfbHf error:check sign Fail!','ERR',true);
