...
class HeShengYuanNewPayController extends PayController
{
    public function Pay($array)
    {
        $body = I('request.pay_productname');//
        $parameter = [
            'code' => 'HeShengYuanNewPay',
            'title' => '合盛元新支付',
            'exchange' => 1,
            'gateway' => '',
            'orderid' => '', //商户订单号,
        ...
        Log::record(" HeShengYuanNewPaynotifyurl 'gateway'=" .$return['gateway'], 'ERR', true);
...
// 异步通知
    public function notifyurl()
    {
        ...
        Log::record(" HeShengYuanNewPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，HeShengYuanNewPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $data = [
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["returncode"])||$response["returncode"] != '00' ){
            Log::record('HeShengYuanNewPay回调 orderid= ' . $response['orderid'] . '  状态失败 $response' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' HeShengYuanNewPay回调 orderid= ' . $response['out_trade_no'] . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
