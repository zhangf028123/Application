...
class UpPddWuXin4Controller extends PayController
{
    public function Pay($array){
        ...
        $senddata =  ['sign' => $sign,
            'context' => $content];
        $gateway =     $return['gateway'].$return["amount"]; 

        $response = HttpClient::post($gateway, $senddata);    //
        $cost_time = $this->msectime() - $start_time;
        Log::record('UpPddWap pay url='.$gateway.'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        $data = [
            ...
            'buyer_user' => $response['buyer_user'],
