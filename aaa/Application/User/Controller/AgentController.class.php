...
class AgentController extends UserController
{
    ...
    {
        parent::__construct();
        if($this->fans['groupid'] == 4) {   // 普通商户不能用这个
            $this->error('没有权限！');
        }
    }
    /**
    ...
    public function userRateEdit()
    {
        ...
        /*$products = M('Product')
            ...
            ->select();*/
        $products = M('Product')
            ->where(['pay_product.status' => 1, 'pay_product.isdisplay' => 1, ])
    ...
    {
        $this->error('您没有权限查对该用户进行费率设置！');  // 20190403 只有管理员能修改
    ...
    {
        $userid = I('get.userid', 0, 'intval'); // pay_member.id
        if(!$userid) {
            $this->error('缺少参数！');
        }
        $member = M('Member')->where(['id'=>$userid])->find();
        ...
        $user_id = [$userid];    // 一级
        $second_id = M('Member')->where(['parentid'=>['in', $user_id]])->getField('id', true) ?: []; // 二级一级member.id
        $third_id = M('Member')->where(['parentid'=>['in', $second_id]])->getField('id', true) ?: []; // 三级级一级member.id
        $pay_memberid = [];
        foreach ([$user_id, $second_id, $third_id] as $onelevel_ids)
            foreach($onelevel_ids as $k => $v) {
                array_push($pay_memberid, $v+10000);
            }

        $where = array('pay_memberid' => ['in', $pay_memberid]);
        //商户号
        ...
        //查询下级数据
        $where['pay_status'] = ['in', '1,2'];   // 只统计成功的
        ...
        //代理分润
        $poundageMap['tcuserid'] = $userid; // 来自这个下级的提成
        ...
        $temp_data  = M('Order')->join('LEFT JOIN __MEMBER__ ON __MEMBER__.id+10000 = __ORDER__.pay_memberid')
        ...
        while($temp_data){
            foreach ($temp_data as $k=>&$v){
                $mid = $v['pay_memberid'] - 10000;  // pay_member.id
                if(in_array($mid, $user_id)){   // 是一级,直接转移
                    $data[$k] = $v;
                    unset($temp_data[$k]);
                    continue;
                }
                $member = M('Member')->where(['id'=>$mid])->find();
                $v['pay_memberid'] = $member['parentid'] + 10000;
            }
        }
    ...
    public function saveUser()
    {
        ...
        $u['groupid'] = I('post.groupid/i'); // 有个bug，前端select不能用数组
        // 创建用户
        $res = M('Member')->add($u);
    ...
    public function order()
    {
        ...
        $successtime = urldecode(I("request.successtime"));
        $memberid = I("request.memberid");  // 下一级代理
        $body = I("request.body");
        $orderid = I("request.orderid");
        // 这里只能看直属下级的，要改为可以看整个下级子树
        if ($memberid) {
        ...
        */
        ...
        $pay_memberid = [];
        $user_id = M('Member')->where(['parentid'=>$this->fans['uid']])->getField('id', true);  // 我的下一级
        $size = 15;
        $rows = I('get.rows', $size, 'intval');
        if (!$rows) {
            $rows = $size;
        ...
        if($user_id) {
            ...
            if(!$createtime and !$successtime) {
                ...
                $stat['todaysum'] = M('Order')->where(['pay_memberid'=>['in', $pay_memberid],"FROM_UNIXTIME(pay_successdate, '%Y%m%d')"=>$today, 'pay_status'=>['in', '1,2']])->sum('pay_amount');
                //今日成功笔数
                $stat['todaysuccesscount'] = M('Order')->where(['pay_memberid'=>['in', $pay_memberid],"FROM_UNIXTIME(pay_successdate, '%Y%m%d')"=>$today, 'pay_status'=>['in', '1,2']])->count();

                $stat['yesterdaysum'] = M('Order')->where(['pay_memberid'=>['in', $pay_memberid],"FROM_UNIXTIME(pay_successdate, '%Y%m%d')"=>$yesterday, 'pay_status'=>['in', '1,2']])->sum('pay_amount');
                //昨日成功笔数
                $stat['yesterdaysuccesscount'] = M('Order')->where(['pay_memberid'=>['in', $pay_memberid],"FROM_UNIXTIME(pay_successdate, '%Y%m%d')"=>$yesterday, 'pay_status'=>['in', '1,2']])->count();
            ...
            $count = M('Order')->where($where)->count();
            $Page  = new Page($count, $rows);
            $temp_data = M('Order')->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->order(['id' => 'desc'])->select();
            // 修改商户编号为一级名字
            $data = [];
            while($temp_data){
                foreach ($temp_data as $k=>&$v){
            ...
            $stat['todaysum'] = $stat['todaysuccesscount'] = $stat['yesterdaysum'] = $stat['yesterdaysuccesscount'] = $stat['totalsum'] = $stat['totalsuccesscount'] = 0;
            $count = 0;
            $Page  = new Page($count, $rows);
            $data = [];
        }
