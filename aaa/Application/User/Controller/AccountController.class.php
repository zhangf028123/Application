...
class AccountController extends UserController
{
    ...
    public function saveProfile()
    {
        if (IS_POST) {
            //用户的ID
            $userid = session('user_auth.uid'); // LoginController::check 存入
            //查询是否开启短信验证
            $sms_is_open = smsStatus();
            $verifysms = 0;
            if ($sms_is_open) {
    ...
    {
        $banklist = M("Systembank")->order('id desc')->select();    // 支付宝微信 排在前面
        ...
        if (IS_POST) {
            ...
            if ($rows['password']) {
                $Member = M('Member');
                $Memberinfo   = $Member->where(['id' => $this->fans['uid']])->find();
                if(md5($rows['password']) != $Memberinfo['paypassword']) {
                    $this->ajaxReturn(['status' => 0, 'msg' => '支付密码错误']);
                }
            } else {
                $this->ajaxReturn(['status' => 0, 'msg' => '支付密码为空']);
            }
            $rows['updatetime'] = time();   //
            if ($id) {
                //$rows['updatetime'] = time();
                $res = D('Common/Bankcard')->where(['id' => $id, 'userid' => $this->fans['uid']])->save($rows);
...
// 上传成功
    ...
    public function channelFinance()
    {
        ...
        foreach ($productList as $k => $v) {
            ...
            $productList[$k]['pay_amount']       = 0.00;    // 交易金额
            $productList[$k]['pay_poundage']     = 0.00;    // 手续费
    ...
    private function getDayReconciliation($begin, $end) {
        ...
        $data['date'] = $date;
        $isMaShang = ($this->fans['collect_type'] == 2);    // 码商
        if($isMaShang){
            $account_ids = array_column( M('ChannelAccount')->where(['memberid'=>$this->fans['uid']])->field('id')->select() ?: [], 'id');
            $where = ['account_id'=>['in', $account_ids],'pay_applydate' => ['between', [strtotime($begin), strtotime($end)]]];
            // 总订单数
            $data['order_total_count'] = M('Order')->where($where)->count();
            //订单总额
            $data['order_total_amount'] = M('Order')->where($where)->sum('pay_amount');

            $where['pay_status'] = ['in', '1,2'];
            //成功订单数
            $data['order_success_count'] = M('Order')->where($where)->count();

            $where['pay_status'] = 0;
            //未支付订单数
            $data['order_fail_count'] = M('Order')->where($where)->count();
            //订单未付总额
            $data['order_success0_amount'] = M('Order')->where($where)->sum('pay_amount');

            $where = ['account_id'=>['in', $account_ids],'pay_successdate' => ['between', [strtotime($begin), strtotime($end)]],  'pay_status' => ['in', '1,2'], ];

            //订单实付总额
            $data['order_success_amount'] = M('Order')->where($where)->sum('pay_amount');
            //费率总额
            $data['order_poundage_amount'] = M('Order')->where($where)->sum('pay_poundage');   // 手续费总额
            ...
            if(empty($data) || diffBetweenTwoDays(date('Y-m-d'),$date)<=30) {//30天内账单实时更新数据
                ...
                //订单实付总额
                $data['order_success_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_successdate' => ['between', [strtotime($begin), strtotime($end)], 'pay_status' => ['in', '1,2']]])->sum('pay_amount');
                //订单未付总额
                $data['order_success0_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_applydate' => ['between', [strtotime($begin), strtotime($end)]], 'pay_status' =>0])->sum('pay_amount');
                //费率总额
                $data['order_poundage_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_successdate' => ['between', [strtotime($begin), strtotime($end)], 'pay_status' => ['in', '1,2']]])->sum('pay_poundage');   // 手续费总额
    ...
    public function tdbalanceOrder()
    {
        ...
        $count = M('TdbalanceOrder')->where($where)->join('LEFT JOIN __ORDER__ ON __TDBALANCE_ORDER__.orderid = __ORDER__.id')->count();
        $page  = new Page($count, 15);
        ...
        $this->assign("page", $page->show());
        C('TOKEN_ON', false);
        $this->display('Admin@User/tdbalanceOrder');
    }
}
