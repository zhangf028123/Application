...
class OrderController extends BaseController
{
    ...
    public function index()
    {
        ...
        $upstream_order_isnull = I("request.upstream_order_isnull");
        if ($upstream_order_isnull != "") {
            if ($upstream_order_isnull == '1') {
                $where['O.upstream_order'] = array('exp', ' is null');
            } else {
                $where['O.upstream_order'] = array('exp', 'is not null');
            }
        }
        $this->assign('upstream_order_isnull', $upstream_order_isnull);
        ...
        $field                    = ['sum(`pay_amount`) pay_amount','sum(`cost`) cost', 'sum(`pay_poundage`) pay_poundage', 'sum(`pay_actualamount`) pay_actualamount', 'count(`id`) success_count', 'sum(`pay_profit`) pay_profit'];
        ...
        $sum['pay_poundage'] = $sum['pay_poundage'] - $sum['cost'] - $sum['memberprofit'];
        foreach ($sum as $k => $v) {
            $sum[$k] += 0;
        }
        //统计订单信息
        ...
        $this->assign('pay_profit', $sum['pay_profit']);
    ...
    public function show()
    {
        ...
        if ($id) {
            $order = M('Order')
                ...
                ->where(['pay_order.pay_orderid' => $id])
                ->find();
        }
    ...
    public function changeRecord()
    {
        ...
        $list = M('Moneychange')
            ->where($where)
            ->join('left join __ORDER__ O on __MONEYCHANGE__.transid = O.pay_orderid')
            ->field('pay_moneychange.*,O.account_id')
            ->limit($page->firstRow . ',' . $page->listRows)
            ->order('id desc')
            ->select();
        if($bank == 9) {
    ...
    public function setOrderPaid() {
        ...
        if(IS_POST) {
            ...
            //Log::record("已禁用 强制设置为已支付: uid = {$uid}, orderid={$order['pay_orderid']},ip={$_SERVER['REMOTE_ADDR']}",'ERR',true);
            //$this->ajaxReturn(['status' => 0, 'msg' => "参数错误！"]);

            /*$payModel = D('Pay');
            $res = $payModel->completeOrder($order['pay_orderid'], '', 0);*/
            $res = R('Pay/Pay/EditMoney', [$order['pay_orderid'], '', 0, ]);
            // 记录日志
            Log::record("强制设置为已支付: uid = {$uid}, orderid={$order['pay_orderid']},ip={$_SERVER['REMOTE_ADDR']}, \$res=$res",'ERR',true);
            if ($res) {
