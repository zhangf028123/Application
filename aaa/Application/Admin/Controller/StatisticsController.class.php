...
{
    const TMT = 7776000; //三个月的总秒数 30天*3
    ...
    public function index()
    {
        ...
        if ($memberid) {
            $where['O.pay_memberid'] = array('eq', $memberid);  // 加过10000的
        ...
        if ($tongdao) {
            $where['O.pay_tongdao'] = array('eq', $tongdao);// Pay下哪个控制器
        }
        ...
        if ($successtime) {
            ...
            $profitMap['datetime'] = ['between', [$cstime, $cetime ? $cetime : date('Y-m-d H:i:s')]];
        } else if (!$successtime && !$createtime) { // 当月的
        ...
        foreach ($list as $item) {
            if ($item['pay_status'] >= 1) {
                $amount += $item['pay_amount']; // 累计订单交易金额
                $rate += $item['pay_poundage']; // 累计订单手续费
                $realmoney += $item['pay_actualamount'];    // 商户到账金额 $pay_shijiamount = $pay_amount - $pay_sxfamount;
            }
        }
       ...
       $sum['memberprofit1'] = M('moneychange')->where(['lx'=>9])->sum('money'); //修复bug 原bug不显示代理总分成
        ...
        $sum['pay_poundage'] = $sum['pay_poundage'] - $sum['cost'] - $sum['memberprofit1']; // 手续费-成本-提成
        foreach($sum as $k => $v) {
            $sum[$k] +=0;
        }
        //统计订单信息
        ...
        $time       = $successtime ? 'pay_successdate' : 'pay_applydate';   // 订单生成还是支付完成的时间
        $dateFormat = $is_month ? '%Y年-%m月' : '%Y年-%m月-%d日';    // 按月还是按日
        ...
        $mdata      = [];
        foreach ($_mdata as $item) {    // amount = pay_actualamount + rate
        ...
        $this->assign("list", $list);   // 订单列表
        $this->assign("mdata", $mdata); // 按日/月统计的数据
        $this->assign('page', $page->show());
        $this->assign('stamount', $sum['pay_amount']);
        $this->assign('stamount1', $sum['pay_amount']);//新总入金   已支付订单总额
        $this->assign('strate', $sum['pay_poundage']);
        $this->assign('strate1', $sum['pay_poundage1']); //新平台总分成 平台收入（含代理分成）总额
        $this->assign('strealmoney', $sum['pay_actualamount']); // 商户收入总金额
        $this->assign('success_count', $sum['success_count']);  // 成功总订单数
        $this->assign('fail_count', $sum['fail_count']);    // 不显示
        $this->assign('memberprofit', $sum['memberprofit']);    // 不显示
        $this->assign('memberprofit1', $sum['memberprofit1']);//新代理分成
        $this->assign('complaints_deposit_freezed', $sum['complaints_deposit_freezed']);    // 不显示
        $this->assign('complaints_deposit_unfreezed', $sum['complaints_deposit_unfreezed']);    // 不显示
    ...
    public function userFinance()
    {
        ...
        if ($memberList) {
            ...
            foreach ($memberList as $k => $v) {
                ...
                $payMemberid = [];
                //if($groupid != 'agent')$payMemberid = [$v['id'] + 10000,];
                // 统计所有下级的
                $user_id = [$v['id']];
                $second_ids = array_column($Member->where(['parentid'=>['in', $user_id]])->field('id')->select(), 'id') ?: [];
                $third_ids = array_column($Member->where(['parentid'=>['in', $second_ids]])->field('id')->select(), 'id') ?: [];
                $fourth_ids = array_column($Member->where(['parentid'=>['in', $third_ids]])->field('id')->select(), 'id') ?: [];
                foreach([$user_id, $second_ids, $third_ids, $fourth_ids] as $ids)
                    foreach ($ids as $id)
                        array_push($payMemberid, $id + 10000);
                $payMemberid = ['in', $payMemberid];
                ...
                $wttklistList = $Wttklist
                    ...
                    ->where(['userid' => $v['id'], 'status' => 2])  // status 1:申请成功！,2:代付成功,4:代付失败！
                    ->find();
                ...
                // 商户自己的id
                $memberList[$k]['pay_memberid'] = $v['id'] + 10000;
                //提交订单数
                ...
                //入金手续费
                $memberList[$k]['pay_poundage'] = $Order->where(['pay_memberid' => $payMemberid, 'pay_status'=>['in', '1,2']])->sum('pay_poundage');
                $memberList[$k]['pay_poundage'] +=0;

                //入金总额
                if($groupid == 'member'){
                    $memberList[$k]['pay_actualamount'] = $Order->where(['pay_memberid' => $payMemberid, 'pay_status'=>['in', '1,2']])->sum('pay_actualamount');
                }
                ...
                if($groupid == 'agent'){  // 代理 入金增额
                    $memberList[$k]['pay_actualamount'] = $profitSum;
                }
                $memberList[$k]['pay_actualamount'] +=0;
        ...
        {   // 代理的不用统计，因为有重复
            ...
            $fields = ['tkmoney', 'sxfmoney', 'money' , 'all_order_count', 'paid_order_count', 'nopaid_order_count', 'all_order_amount', 'paid_order_amount', 'member_income', 'platform_income','balance','blockedbalance','pay_amount',
                'tkmoney','sxfmoney', 'pay_poundage', 'pay_actualamount', ];
    ...
    public function exportUserFinance(){
        ...
        $stat['member_count'] = count($memberList);
        $fields = [ 'all_order_count', 'paid_order_count', 'nopaid_order_count', 'all_order_amount', 'paid_order_amount', 'pay_poundage',   // order
            'tkmoney', 'sxfmoney', 'money' ,    // tklist
            'member_income', 'platform_income','balance','blockedbalance','pay_amount',];
        foreach($fields as $field ) {
            foreach($memberList as $k => $v) {
                $stat[$field] += $v[$field];
            }
    ...
    public function channelFinance()
    {
        ...
        for ($i = 0; $i < $orderCount; $i += $limit) {
            ...
            $tempList = $Order
                ->field(['pay_bankcode', 'pay_amount', 'pay_poundage', 'pay_actualamount', 'pay_status', 'pay_applydate'])
                ->limit($i, $limit)
                ->select();
        ...
        foreach ($productList as $k => $v) {
            ...
            $productList[$k]['today_count']            = 0;
            $productList[$k]['today_fail_count']       = 0;
            $productList[$k]['today_success_count']    = 0;
            $productList[$k]['today_success_rate']     = 0;
            $productList[$k]['today_pay_amount']       = 0.00;
            $productList[$k]['today_pay_poundage']     = 0.00;
            $productList[$k]['today_pay_actualamount'] = 0.00;
            ...
            foreach ($orderList as $k1 => $v1) {
                if ($v['id'] == $v1['pay_bankcode']) {
                    ...
                    //判断同一天
                    if (!$this -> isDiffDays(getdate($v1['pay_applydate']), getdate(time()))) {
                        $productList[$k]['today_count']++;
                        if ($v1['pay_status'] != 0) {
                            $productList[$k]['today_success_count']++;
                            $productList[$k]['today_pay_amount']       = bcadd($productList[$k]['today_pay_amount'], $v1['pay_amount'], 4);
                            $productList[$k]['today_pay_poundage']     = bcadd($productList[$k]['today_pay_poundage'], $v1['pay_poundage'], 4);
                            $productList[$k]['today_pay_actualamount'] = bcadd($productList[$k]['today_pay_actualamount'], $v1['pay_actualamount'], 4);
                        }
                    }
                }
            }
            ...
            $productList[$k]['today_fail_count']   = $productList[$k]['today_count'] - $productList[$k]['today_success_count'];
            $productList[$k]['today_success_rate'] = bcdiv($productList[$k]['today_success_count'], $productList[$k]['today_count'], 4) * 100;
    ...
    */
    ...
    public function merchantReport() {
        ...
        $data = $Member
            ...
            ->select();
        if ($data) {
            $stat['initial_money'] = 0.00;
            $stat['income_money'] = 0.00;
            $stat['pay_wait_checked'] = 0.00;
            $stat['pay_success'] = 0.00;
            $stat['pay_fail'] = 0.00;
            $stat['frozen_money'] = 0.00;
            $stat['merchant_money'] = 0.00;
            $stat['current_money'] = 0.00;
            $stat['end_profit'] = 0.00;
            ...
            foreach ($data as $k => $v) {
                ...
                $stat['initial_money'] = bcadd($stat['initial_money'], $data[$k]['initial_money'], 4);
                $stat['income_money'] = $stat['income_money'] + $data[$k]['income_money'];
                $stat['pay_wait_checked'] = $stat['pay_wait_checked'] + $data[$k]['pay_wait_checked'];
                $stat['pay_success'] = $stat['pay_success'] + $data[$k]['pay_success'];
                $stat['pay_fail'] = $stat['pay_fail'] + $data[$k]['pay_fail'];
                $stat['frozen_money'] = $stat['frozen_money'] + $data[$k]['frozen_money'];
                $stat['merchant_money'] = $stat['merchant_money'] + $data[$k]['merchant_money'];
                $stat['current_money'] = $stat['current_money'] + $data[$k]['current_money'];
                $stat['end_profit'] = $stat['end_profit'] + $data[$k]['end_profit'];
                ...
                if ($data[$k]['initial_money'] == 0 && $data[$k]['income_money'] == 0) {
                    unset($data[$k]);
                }
            }
        }
   ...
   */
    private function getDateBalance($userid, $date) {
        ...
        $log = M('Moneychange')->where(['userid'=>$userid, 'datetime'=>array('elt', $date), 't'=>['neq', 1], 'lx' => ['not in', '3,4']])->order('datetime DESC,id DESC')->find();   // $date前最后一条记录
        if(empty($log)) {
            $money = 0;
        } else {
            $yesterdayTime = date("Y-m-d 00:00:00",strtotime($date)-1);
            $yesterdayRedAddSum = M('redo_order')->where(['type'=>1,'user_id'=>$userid,'date'=>$yesterdayTime, 'ctime'=>['gt', strtotime($log['datetime'])]])->sum('money');// $date一天前的记录
            ...
            if($lastlog['lx'] == 3 || $lastlog['lx'] == 4) {
                $money = $lastlog['gmoney'];    // 管理员手动变动后的金额
            } else {
    ...
    public function profitFinance()
    {
     ...
     //   $rows = I('get.rows', $size, 'intval');
        ...
        $count = $Member->where(['collect_type' => '2']) -> count();
        $Page  = new Page($count, $rows);
        $show  = $Page->show();
               // \Think\Log::record('11111111111'.$show.'--'.$count, 'ERR', true);
        ...
        // 今天时间
        $todayBegin = $date.' 00:00:00';
        $todyEnd = $date.' 23:59:59';
        $yesterdayBegin = (new Date($todayBegin))->dateAdd(-1)->format();
        //$yesterdayEnd = (new Date($todyEnd))->dateAdd(-1)->format();

        $Order      = M('Order');
        $orderList = $Order
            ->field(['pay_bankcode', 'account_id', 'pay_amount', 'pay_profit', 'pay_status', 'pay_applydate', 'margins', 'pay_device'])
            ->where([ 'pay_applydate' => ['between', [strtotime($yesterdayBegin), strtotime($todyEnd)]]])
            ->select();

        // dump($orderList);exit;
        //处理查询的数据

        $stat['today_pay_total_amount']       = 0.00;
        $stat['today_pay_amount']     = 0.00;
        $stat['today_pay_profit'] = 0.00;
        $stat['pay_total_amount']       = 0.00;
        $stat['pay_amount']     = 0.00;
        $stat['pay_profit'] = 0.00;

        foreach ($MemberList as $k => $v) {

            $my_account_ids = array_column(M('channel_account')->where(['memberid'=>$v['id']])->field('id')->select() , 'id');

            $MemberList[$k]['today_count']            = 0;
            $MemberList[$k]['today_fail_count']       = 0;
            $MemberList[$k]['today_success_count']    = 0;
            $MemberList[$k]['today_success_rate']     = 0;
            $MemberList[$k]['today_pay_total_amount']       = 0.00;
            $MemberList[$k]['today_pay_amount']     = 0.00;
            $MemberList[$k]['today_pay_profit'] = 0.00;
            $MemberList[$k]['today_device_ios'] = 0;
            $MemberList[$k]['today_device_android'] = 0;
            $MemberList[$k]['today_device_other'] = 0;
            $MemberList[$k]['today_success_device_ios'] = 0;
            $MemberList[$k]['today_success_device_android'] = 0;
            $MemberList[$k]['today_success_device_other'] = 0;

            $MemberList[$k]['count']            = 0;
            $MemberList[$k]['fail_count']       = 0;
            $MemberList[$k]['success_count']    = 0;
            $MemberList[$k]['success_rate']     = 0;
            $MemberList[$k]['pay_total_amount']       = 0.00;
            $MemberList[$k]['pay_amount']     = 0.00;
            $MemberList[$k]['pay_profit'] = 0.00;
            $MemberList[$k]['device_ios'] = 0;
            $MemberList[$k]['device_android'] = 0;
            $MemberList[$k]['device_other'] = 0;
            $MemberList[$k]['success_device_ios'] = 0;
            $MemberList[$k]['success_device_android'] = 0;
            $MemberList[$k]['success_device_other'] = 0;

            foreach ($orderList as $k1 => $v1) {
                if (in_array($v1['account_id'], $my_account_ids)) {
                    //判断同一天
                    if (!$this -> isDiffDays(getdate($v1['pay_applydate']), getdate(strtotime($todyEnd)))) {
                        if ($v1['pay_device'] == 'ios') {
                            $MemberList[$k]['today_device_ios']++;
                        } elseif ($v1['pay_device'] == 'android') {
                            $MemberList[$k]['today_device_android']++;
                        } else {
                            $MemberList[$k]['today_device_other']++;
                        }
                        $MemberList[$k]['today_count']++;
                        $stat['today_pay_total_amount'] = bcadd($stat['today_pay_total_amount'], $v1['pay_amount'], 4);
                        $MemberList[$k]['today_pay_total_amount'] = bcadd($MemberList[$k]['today_pay_total_amount'], $v1['pay_amount'], 4);
                        if ($v1['pay_status'] != 0) {
                            $stat['today_pay_amount']       = bcadd($stat['today_pay_amount'], $v1['pay_amount'], 4);
                            $stat['today_pay_profit']     = bcadd($stat['today_pay_profit'], $v1['margins'], 4);

                            if ($v1['pay_device'] == 'ios') {
                                $MemberList[$k]['today_success_device_ios']++;
                            } elseif ($v1['pay_device'] == 'android') {
                                $MemberList[$k]['today_success_device_android']++;
                            } else {
                                $MemberList[$k]['today_success_device_other']++;
                            }

                            $MemberList[$k]['today_success_count']++;
                            $MemberList[$k]['today_pay_amount']       = bcadd($MemberList[$k]['today_pay_amount'], $v1['pay_amount'], 4);
                            //$pay_profit = $v1['pay_profit'] - $v1['provider_cost'];
                            $MemberList[$k]['today_pay_profit']     = bcadd($MemberList[$k]['today_pay_profit'], $v1['margins'], 4);
                        }
                    } else {
                        $MemberList[$k]['count']++;
                        $stat['pay_total_amount'] = bcadd($stat['pay_total_amount'], $v1['pay_amount'], 4);
                        if ($v1['pay_device'] == 'ios') {
                            $MemberList[$k]['device_ios']++;
                        } elseif ($v1['pay_device'] == 'android') {
                            $MemberList[$k]['device_android']++;
                        } else {
                            $MemberList[$k]['device_other']++;
                        }

                        $MemberList[$k]['pay_total_amount'] = bcadd($MemberList[$k]['pay_total_amount'], $v1['pay_amount'], 4);
                        if ($v1['pay_status'] != 0) {
                            $stat['pay_amount']       = bcadd($stat['pay_amount'], $v1['pay_amount'], 4);
                            $stat['pay_profit']     = bcadd($stat['pay_profit'], $v1['margins'], 4);

                            if ($v1['pay_device'] == 'ios') {
                                $MemberList[$k]['success_device_ios']++;
                            } elseif ($v1['pay_device'] == 'android') {
                                $MemberList[$k]['success_device_android']++;
                            } else {
                                $MemberList[$k]['success_device_other']++;
                            }


                            $MemberList[$k]['success_count']++;
                            $MemberList[$k]['pay_amount']       = bcadd($MemberList[$k]['pay_amount'], $v1['pay_amount'], 4);
                            //$pay_profit = $v1['pay_profit'] - $v1['provider_cost'];
                            $MemberList[$k]['pay_profit']     = bcadd($MemberList[$k]['pay_profit'], $v1['margins'], 4);
                        }

                    }
                }
            }
            $MemberList[$k]['fail_count']   = $MemberList[$k]['count'] - $MemberList[$k]['success_count'];
            $MemberList[$k]['success_rate'] = bcdiv($MemberList[$k]['success_count'], $MemberList[$k]['count'], 4) * 100;
            $MemberList[$k]['today_fail_count']   = $MemberList[$k]['today_count'] - $MemberList[$k]['today_success_count'];
            $MemberList[$k]['today_success_rate'] = bcdiv($MemberList[$k]['today_success_count'], $MemberList[$k]['today_count'], 4) * 100;
            if ($MemberList[$k]['count'] == 0 && $MemberList[$k]['today_count'] == 0) {
                unset($MemberList[$k]);
            }
        }
        $this->assign('date', $date);
    ...
    public function merchantprofit() {
        ...
        if ($data) {
            ...
            foreach ($data as $k => $v) {
                $data[$k]['memberid'] = $v['id'] +10000;
                $data[$k]['username'] = $v['username'];
                $data[$k]['aliasname'] = $v['aliasname'];
                //期初余额
                $data[$k]['initial_money'] = $this->getDateBalance($v['id'],$beginDate);
                //期末余额
                $data[$k]['end_profit'] = $this->getDateBalance($v['id'],$endDate);

                $data[$k]['initial_money'] = sprintf('%.4f',$data[$k]['initial_money']);
                $data[$k]['end_profit'] = sprintf('%.4f',$data[$k]['end_profit']);

                $stat['initial_money'] = bcadd($stat['initial_money'], $data[$k]['initial_money'], 4);
                $stat['end_profit'] = bcadd($stat['end_profit'], $data[$k]['end_profit'], 4);

                if ($data[$k]['initial_money'] == 0 && $data[$k]['end_profit'] == 0) {
                    unset($data[$k]);
                }
            }
        }
        foreach($data as $data1){
            $sort[]=$data1["end_profit"];
        }
