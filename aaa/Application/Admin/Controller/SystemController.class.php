...
class SystemController extends BaseController
{
    ...
    public function editPassword()
    {
        if (IS_POST) {
            ...
            // TODO: 修改session('admin_auth')['password']
...
EOD;
    ...
    public function clearData() {
        \Think\Log::record('clearData ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);
        /*
        $clear_types = [
            '1' => '入金记录(订单表)',
            '2' => '代付记录(提款记录)',
        ...
        $this->assign('clear_types', $clear_types);
        ...
        if(IS_POST) {
            ...
            foreach($type_ids as $v) {
                if($v == 1) { //入金记录
                    ...
                    $res2 = M('wttklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();    // 提款 代付结算
                    $res3 = M('tklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();      // 提款 手动结算
                } elseif($v == 3) {//登录记录
                    $res4 = M('loginrecord')->where(['logindatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                } elseif($v == 4) {//冻结记录
                    $res5 = M('blockedlog')->where(['createtime'=>['between', [$startTime, $endTime]]])->delete();
                ...
                switch ($v){
                    case 6:
                        $res8 = M('account_day_stat')->where(['date'=>['between', [date('Ymd',$startTime), date('Ymd',$endTime)]]])->delete();
                        break;
                    case 7:
                        $res9 = M('tdsxf_order')->where(['ctime'=>['between', [$startTime, $endTime]]])->delete();
                        break;
                    case 8:
                        $res9 = M('tdbalance_order')->where(['ctime'=>['between', [$startTime, $endTime]]])->delete();
                        break;
                    case 9:
                        $res10 = M('amount_water_order')->where(['ctime'=>['between', [$startTime, $endTime]]])->delete();
                        break;
                    case 10:
                        $res11 = M('channel_account')->where(['createtime'=>['between', [$startTime, $endTime]]])->delete();
                        break;
                    default:
                        break;
                }
            ...
            if(FALSE !== $res1 && FALSE !== $res2 && FALSE !== $res3 && FALSE !== $res4 && FALSE !== $res5 && FALSE !== $res6 && FALSE !== $res7
                && FALSE !== $res8 && FALSE !== $res9 && FALSE !== $res10 && FALSE !== $res11
            ) {
                M()->commit();
    ...
    /// 清理金额数据
    public function delAllAmountData(){
        \Think\Log::record('delAllAmountData ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);

        /*
        $sql = <<<SQL
delete from pay_wttklist;
delete from pay_tklist;
-- delete from pay_loginrecord;
delete from pay_blockedlog;
delete from pay_complaints_deposit;
delete from pay_moneychange;

-- 后台没有设置清理数据的
delete from pay_account_day_stat;
delete from pay_tdsxf_order;
delete from pay_tdbalance_order;
delete from pay_arrearage;
delete from pay_amount_water_order;
delete from pay_retire_log;
delete from pay_account_switch_log;

-- 清理会员交易信息
update pay_member set balance=0, blockedbalance=0,
unit_paying_number=0, unit_paying_amount=0, unit_frist_paying_time=0, last_paying_time=0,
paying_money = 0, td_sxf=0, td_balance=0, amount_water=0, dj_amount_water=0;
SQL;

        if(IS_POST) {
            do{
                $res = DatabaseBackup();    // 这里没写入数据，有问题
                if (!$res)break;
                try{
                    //开启事物
                    $res = M()->execute($sql);
                    if($res !== false)$this->ajaxReturn(['status' => 1, 'msg' => "清理成功！"]);
                }catch (Exception $e){
                    break;
                }
            }while(0);
