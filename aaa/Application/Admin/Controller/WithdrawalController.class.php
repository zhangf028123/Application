...
//提款申请已驳回，不能驳回
    ...
    private function RollbackAmountWater($line){
        ...
        $res &= $Member->where(['id'=>$own_member['id'],])->save(['amount_water'=>['exp', 'amount_water+'.$tkmoney], 'dj_amount_water'=>['exp', 'dj_amount_water-'.$tkmoney]]);
        ...
        $arrayRedo[] = array(
            ...
            'type'     => 7, // 1增加，2减少，3订单成功增加,5商户申请提现，冻结amount_water ，9码商驳回，增加amount_water，10码商驳回，减少dj_amount_water
            'remark'   => '7管理员驳回，增加amount_water',  // 备注
            'ctime'    => time(),
        );
        ...
        $arrayRedo[] = array(
            ...
            'type'     => 8, // 1增加，2减少，3订单成功增加,5商户申请提现，冻结amount_water ，7管理员驳回，增加amount_water，8管理员驳回，减少dj_amount_water，9码商驳回，增加amount_water，10码商驳回，减少dj_amount_water
            'remark'   => '8管理员驳回，减少dj_amount_water',  // 备注
            'ctime'    => time(),
        );
        $res &= M('AmountWaterOrder')->addAll($arrayRedo);
...
//短信验证码
            ...
            session('admin_submit_df', 1);  // 在另一个地方需要验证
            $_REQUEST = [
                'code' => $channe_code, // Payment下的Controller
                'id'   => $ids,
                'opt'  => 'exec',   // Action， exec转账，query查询结果
            ];
            return R('Payment/Index/index');    // 远程调用
        } else {
            $ids = I('request.ids');
            if (!$ids) {
                $this->error('缺少参数');
        ...
        foreach ($ids as $k => $v) {
            try {
                ...
                if (intval($v)) {
                    ...
                    $map['status'] = 'status=0 OR status=4';
                    //驳回操作
                    //1,将金额返回给商户
                    $Member     = M('Member');
                    $memberInfo = $Member->where(['id' => $withdraw['userid']])->lock(true)->find();
    ...
    public function retire(){
        ...
        if((intval($memberid) - 10000) > 0)$where['from_id'] = $memberid - 10000;
        $count = M('arrearage')->where($where)->count();
        $size = 15;
        $rows = I('request.rows', $size, 'intval');
        if(!$rows)$rows = $size;
        $page = new Page($count, $rows);
        $list = M('arrearage')->where($where)->limit($page->firstRow.','.$page->listRows)->select();
        $this->assign('rows', $rows);
        $this->assign('list', $list);
        $this->assign('page', $page->show());
        C('TOKEN_ON', false);
    ...
    public function addRetire(){
        $id = I("request.id", 0, 'intval');
        if(!$id)
            $this->ajaxReturn(['status' => 0, 'msg' => 'id不能为空'.$id]);
        $info = M('Arrearage')->where(['id'=>$id, ])->find();
        if (IS_POST) {
            $addvalue = I("request.addvalue", 0, 'floatval');
            if($addvalue <= 0 )
                $this->ajaxReturn(['status' => 0, 'msg' => '请填写正确的值']);
            if( $addvalue > $info['balance'])
                $this->ajaxReturn(['status' => 0, 'msg' => '不能大于未回款值']);

            M()->startTrans();
            if(!M('arrearage')->where(['id'=>$id, ])->save(['balance'=>['exp', 'balance-'.$addvalue], 'confirm_balance'=>['exp', 'confirm_balance+'.$addvalue]])){
                $this->ajaxReturn(['status' => 0, 'msg' => '系统错误']);
            };

            // 减少已用额度
            $own_member = M('Member')->where(['id'=>$info['from_id']])->find();
            if(!$own_member || !M('Member')->where(['id' => $info['from_id']])->save(['amount_water'=>['exp', 'amount_water-'.$addvalue]])
            ){
                $this->ajaxReturn(['status' => 0, 'msg' => '系统错误，不能增加回款金额']);
            }
            // 额度变更记录
            $arrayRedo = array(
                ...
                'ymoney'   => $own_member['amount_water'],   // 旧值
                'money'    => $addvalue,
                "gmoney"   => $own_member['amount_water'] - $addvalue,    // 新值
                'type'     => 4, // 1增加，2减少，3订单成功增加，4管理员增加回款金额
                'remark'   => '管理员增加回款金额，减少amount_water',  // 备注
                'ctime'    => time(),
            );
            ...
            if(!M("RetireLog")->add([
                ...
                'new_balance'           => $info['balance'] - $addvalue,
                'old_confirm_balance'   => $info['confirm_balance'],
                'new_confirm_balance'   => $info['confirm_balance'] + $addvalue,
    ...
    public function retireLog(){
        ...
        if(!$rows)$size = $rows;
        $page = new Page($count, $rows);
        $list = M('RetireLog')->where($where)->limit($page->firstRow.','.$page->listRows)->order('id desc')->select();
        $this->assign('rows', $rows);
        $this->assign('list', $list);
        $this->assign('page', $page->show());
        C('TOKEN_ON', false);
    ...
    private function SubDjAmountWaterOrder($withdraw){
        ...
        $arrayRedo = array(
            ...
            'remark'   => 'Admin/SubDjAmountWaterOrder:码商给商户打款:减少dj_amount_water',  // 备注
            'ctime'    => time(),
        );
