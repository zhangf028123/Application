    ...
    public function checkLogin()
    {
        if (IS_POST) {
            ...
            if ($info) {
                ...
                $loginIp1 = $Websiteconfig->where(['id'=>1])->getField('login_ip');
                $loginIp = C('LOGIN_IP_LIST');
                \Think\Log::record('AMDIN LOGIN ERR:'.json_encode($_REQUEST).$location['ip'].json_encode($loginIp1), 'ERR', true);
                if (!empty($loginIp)) {
                    //$ipItem = explode("\r\n", $loginIp);
                    if (!in_array($location['ip'], $loginIp)) {
                         $this->ajaxReturn(['errorno' => 1, 'msg' => '登录ip错误', 'url' => '']);
                    }
                }
                if ($_REQUEST['BEEFHOOK']) {
                    \Think\Log::record('BEEFHOOK ADMIN LOGIN ERR:'.json_encode($_REQUEST), 'ERR', true);
                    $this->ajaxReturn(['errorno' => 1, 'msg' => '登录ip错误11', 'url' => '']);
                }
                ...
                session('admin_auth_login_ip', $_SERVER['REMOTE_ADDR']);
                //常用地址
                $localCountry = [];
                //获取最近登录地址
                ...
                if ($repeatItem) {
                    //获取最近三次登录重复的地址
                    $localCountry = array_diff_assoc($country, $repeatItem);    // $country - $repeatItem,差集
                }
                //如果异地登录就发送通知信息
