...
class AdminController extends BaseController
{
    ...
    public function check_cross($from){
        $referer_host = parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST);
        $domain_host = parse_url( 'http://'.C('DOMAIN'), PHP_URL_HOST);
        if ( $referer_host !==  $domain_host) {  // 仅仅比较主机名
            Log::record("check_cross $from:$referer_host !== $domain_host ", 'ERR',true);
            $this->ajaxReturn(['status' => 0, 'msg' => 'Denied']);
        }
    }
    public function addAdmin(){
        \Think\Log::record('addAdmin ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);

        /*
        if(IS_POST){
            //防止跨站请求伪造
            $this->check_cross('addAdmin');
            if(UID != 1){   // 只有admin能添加管理员
                // $this->ajaxReturn(['status' => 0, 'msg' => 'Denied, 只有admin能添加管理员']);
            }
            ...
            Log::record("uid=".UID.",添加管理员：{$data["username"]} ,result={$add_admin_result},request=".json_encode($_REQUEST), 'ERR',true);
    ...
    {
        \Think\Log::record('deleteAdmin ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);

        /*
        //防止跨站请求伪造
        $this->check_cross('deleteAdmin');
        $id = I('id', 0, 'intval');
        if(!$id){
    ...
    public function editAdmin(){
        \Think\Log::record('editAdmin ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);

        /*
        if(IS_POST){
            //防止跨站请求伪造
            $this->check_cross('editAdmin');
            ...
            if($admin_result!==false){
                ...
                $admin_auth_group_access_model->changeGroup($data['id'], $data["groupid"]);
