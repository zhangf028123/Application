...
class UserController extends BaseController
{
    ...
    {
        \Think\Log::record('delUser ATTACK ERR:'.json_encode($_REQUEST), 'ERR', true);
    ...
    /// 额度变动记录
    public function AmountWaterOrder(){
        $where    = array();
        $memberid = I("get.uid");
        if ($memberid) {
        ...
        if ($createtime) {
            ...
            $where['ctime']    = ['between', [strtotime($cstime), strtotime($cetime ? $cetime : date('Y-m-d'))]];
        }
        $count = M('AmountWaterOrder')->where($where)//->join('LEFT JOIN __ORDER__ ON __AMOUNT_WATER_ORDER__.orderid = __ORDER__.id')
        ->count();
        $page  = new Page($count, 15);
        $list  = M('AmountWaterOrder')->alias('a')->join('LEFT JOIN __ORDER__ as o ON a.orderid = o.id')
            ->where($where)
            ->limit($page->firstRow . ',' . $page->listRows)
            ->order('id desc')
            ->field('a.*,o.pay_orderid')
            ->select();
        /*$sql  = M('AmountWaterOrder')->alias('a')->join('LEFT JOIN __ORDER__ as o ON a.orderid = o.id')
            ->where($where)
            ->limit($page->firstRow . ',' . $page->listRows)
            ->order('id desc')
            ->field('a.*,o.pay_orderid')
    ...
    public function incrMoney()
    {
        ...
        if (IS_POST) {
            ...
            $bgmoney    = I("post.bgmoney",0,'float');  // bian geng money
            ...
            $arrayField  = array(
                "userid"     => $userid,
                'ymoney'     => $info['balance'],   // 旧值
                "money"      => $bgmoney,   // bian geng money
                "gmoney"     => $gmoney,    // 新值
                "datetime"   => date("Y-m-d H:i:s"),    // 变更日期
                "tongdao"    => '',
                "transid"    => "",
                "orderid"    => "",
                "lx"         => $cztype, // 增减类型，3增加，4减少
    ...
    public function incrTdSxf()
    {
        ...
        if (IS_POST) {
            ...
            if (strtotime($date) > time()) {
                $this->ajaxReturn(['status' => 0, 'msg' => '日期不正确']);
            }
            $info = M("Member")->where(["id" => $userid])->lock(true)->find();
            if (empty($info)) {
                $this->ajaxReturn(['status' => 0, 'msg' => '用户不存在']);
            }
            if (($info['td_sxf'] - $bgmoney) < 0 && $cztype == 2) {
                $this->ajaxReturn(['status' => 0, 'msg' => "预存手续费不足" . $bgmoney . "元，不能完成减少操作"]);
            }
            if ($cztype == 1) { // 加余额
                $data["td_sxf"] = array('exp', "td_sxf+" . $bgmoney);
                $gmoney         = $info['td_sxf'] + $bgmoney;
            } elseif ($cztype == 2) {   // 减余额
                $data["td_sxf"]  = array('exp', "td_sxf-" . $bgmoney);
                $where['td_sxf'] = array('egt', $bgmoney);
                $gmoney          = $info['td_sxf'] - $bgmoney;
            }
            $where['id'] = $userid;
            $res1        = M('Member')->where($where)->save($data);
            ...
            $arrayRedo = array(
                'user_id'  => $userid,
                'orderid'   => '', // 订单表的id，扣手续费的时候用
                'admin_id' => session('admin_auth')['uid'],
                'ymoney'   => $info['td_sxf'],   // 旧值
                'money'    => $bgmoney,
                "gmoney"   => $gmoney,    // 新值
                'type'     => $cztype, // 1增加，2减少，3订单成功消耗
                'remark'   => $contentstr . '【调整预存手续费:' . $date . '】',  // 备注
                'ctime'    => time(),
            );
    ...
    public function incrTdBalance()
    {
        ...
        if (IS_POST) {
            ...
            M()->startTrans();
            $userid     = I("post.uid", 0, 'intval');
            //$rmb        = I("post.rmb", 0, 'intval');
            $cztype     = I("post.cztype");
            $bgmoney    = I("post.bgmoney",0,'float');  // bian geng money
            $contentstr = I("post.memo", "");
            $auth_type  = I('post.auth_type', 0, 'intval');
            /*if ($rmb <= 0) {
                $this->ajaxReturn(['status' => 0, 'msg' => "人民币必须是正数！"]);
            }*/
            if ($bgmoney <= 0) {
                $this->ajaxReturn(['status' => 0, 'msg' => "变动金额必须是正数！"]);
            }
            ...
            if (($info['td_balance'] - $bgmoney) < 0 && $cztype == 2) {
                $this->ajaxReturn(['status' => 0, 'msg' => "押金不足" . $bgmoney . "元，不能完成减少操作"]);
            }
            if ($cztype == 1) { // 加余额
                $data["td_balance"] = array('exp', "td_balance+" . $bgmoney);
                $gmoney         = $info['td_balance'] + $bgmoney;
            } elseif ($cztype == 2) {   // 减余额
                $data["td_balance"]  = array('exp', "td_balance-" . $bgmoney);
                $where['td_balance'] = array('egt', $bgmoney);
                $gmoney          = $info['td_balance'] - $bgmoney;
            }
            $where['id'] = $userid;
            $res1        = M('Member')->where($where)->save($data);
            ...
            $arrayRedo = array(
                ...
                'admin_id' => session('admin_auth')['uid'],
               // 'rmb'      => $rmb,
                'ymoney'   => $info['td_balance'],   // 旧值
                'money'    => $bgmoney,
                "gmoney"   => $gmoney,    // 新值
                'type'     => $cztype, // 1增加，2减少，3订单成功消耗
                'remark'   => $contentstr . '【调整押金:' . $date . '】',  // 备注
                'ctime'    => time(),
            );
            $res2 = M('TdbalanceOrder')->add($arrayRedo);
            if ($res1 && $res2) {
                M()->commit();
                $this->ajaxReturn(['status' => 1, 'msg' => '操作成功']);
            } else {
     ...
     * 管理预存手续费
     * author: luofei
     * create: 2019/03/17 12:22
     */
    public function tdsxfOrder()
    {
        $where    = array();
        $memberid = I("get.uid");
        if ($memberid) {
        ...
        $count = M('TdsxfOrder')->where($where)->join('LEFT JOIN __ORDER__ ON __TDSXF_ORDER__.orderid = __ORDER__.id')->count();
        $page  = new Page($count, 15);
        $list  = M('TdsxfOrder')
            ->where($where)->join('LEFT JOIN __ORDER__ ON __TDSXF_ORDER__.orderid = __ORDER__.id')
            ->limit($page->firstRow . ',' . $page->listRows)
            ->order('pay_tdsxf_order.id desc')
            ->field('pay_tdsxf_order.*,pay_order.pay_orderid')
            ->select();
        $this->assign("list", $list);
        $this->assign("page", $page->show());
        C('TOKEN_ON', false);
     ...
     * create: 2019/03/17 12:22
     */
    public function tdbalanceOrder()
    {
        $where    = array();
        $memberid = I("get.uid");
        if ($memberid) {
        ...
        $count = M('TdbalanceOrder')->where($where)//->join('LEFT JOIN __ORDER__ ON __TDBALANCE_ORDER__.orderid = __ORDER__.id')
            ->count();
        $page  = new Page($count, 15);
        $list  = M('TdbalanceOrder')
            ->where($where)//->join('LEFT JOIN __ORDER__ ON __TDBALANCE_ORDER__.orderid = __ORDER__.id')
            ->limit($page->firstRow . ',' . $page->listRows)
            ->order('pay_tdbalance_order.id desc')
            //->field('pay_tdbalance_order.*,pay_order.pay_orderid')
            ->select();
        $this->assign("list", $list);
        $this->assign("page", $page->show());
        C('TOKEN_ON', false);
    ...
    public function saveUser()
    {
        if (IS_POST) {
            ...
            if($u['parentid'] && $u['parentid'] != 1 && $u['parentid'] > 10000){
                $u['parentid'] -= 10000;
            }

            if ($userid) {
                unset($u['collect_type']);  // 已有用户，不能修改此字段
                ...
                // 创建用户
                $res = M('Member')->add($u);
                // 20190406新建的用户，费率默认100‰
                {
                    $userid = $res; // 新建的角色id
                    $rows   = M('Product')->select();
                    foreach ($rows as $key => $item) {
                        $rates = M('Userrate')->where(['userid' => $userid, 'payapiid' => $item['id']])->find();
                        $default_feilv = 0.05;  // 新用户默认费率
                        if ($rates) {
                            $data_insert[] = ['id' => $rates['id'], 'userid' => $userid, 'payapiid' => $item['id'], 'feilv' => $default_feilv, 'fengding' => $default_feilv, 't0feilv' => $default_feilv, 't0fengding' => $default_feilv];
                        } else {
                            $data_update[] = [                      'userid' => $userid, 'payapiid' => $item['id'], 'feilv' => $default_feilv, 'fengding' => $default_feilv, 't0feilv' => $default_feilv, 't0fengding' => $default_feilv];
                        }
                    }
    ...
    public function userRateEdit()
    {
        ...
        if ($products) {
            foreach ($products as $key => $item) {
                ...
                $products[$key]['t0feilv_night']    = $_tmpData[$item['id']]['t0feilv_night'] ? $_tmpData[$item['id']]['t0feilv_night'] : '0.0000';
                $products[$key]['t0fengding_night'] = $_tmpData[$item['id']]['t0fengding_night'] ? $_tmpData[$item['id']]['t0fengding_night'] : '0.0000';
            }
        }
    ...
    public function saveUserRate()
    {
        if (IS_POST) {
            ...
            foreach ($rows as $key => $item) {
                ...
                foreach (['feilv', 'fengding', 't0feilv', 't0fengding', 't0feilv_night', 't0fengding_night',] as $key1){
                    if (isset($rows[$key1]) && $rows[$key1] > 1)$this->ajaxReturn(['status'=>0, 'msg'=>"费率{$key1}不能大于1"]);
                }
                if ($rates) {
                    $data_insert[] = ['id' => $rates['id'], 'userid' => $userid, 'payapiid' => $key, 'feilv' => $item['feilv'], 'fengding' => $item['fengding'], 't0feilv' => $item['t0feilv'], 't0fengding' => $item['t0fengding'], 't0feilv_night' => $item['t0feilv_night'], 't0fengding_night' => $item['t0fengding_night'],];
                } else {
                    $data_update[] = ['userid' => $userid, 'payapiid' => $key, 'feilv' => $item['feilv'], 'fengding' => $item['fengding'], 't0feilv' => $item['t0feilv'], 't0fengding' => $item['t0fengding'], 't0feilv_night' => $item['t0feilv_night'], 't0fengding_night' => $item['t0fengding_night'],];
                }
            }
            M('Userrate')->addAll($data_insert, [], true);
            M('Userrate')->addAll($data_update, [], true);
    ...
    {
        $userid = I('get.uid', 0, 'intval');    // memberid
        //系统产品列表
        $products = M('Product')
            ->where(['isdisplay' => 1])
    ...
    public function saveUserProduct()
    {
        if (IS_POST) {
            $userid = I('post.userid', 0, 'intval');
            $u      = I('post.u/a');
            \Think\Log::record('testing..'.json_encode($u), 'ERR', true);
            if ($userid == 61) {
                //指定商户 全部替换
                foreach ($u as $key => $item) {
                    if ($item['channel']) {
                        ...
                        M('Product_user')->where(array('pid' => $key))->save(array('status' => $status, 'polling' => $item['polling'], 'channel' => $item['channel'], 'weight' => trim($weightStr, '|')));
                    }
                }
                $this->ajaxReturn(['status' => 1]);
    ...
    public function agentList()
    {
        ...
        if (!empty($parentid) && !is_numeric($parentid)) {  // 用户名
            $User              = M("Member");
            $pid               = $User->where(['username' => $parentid])->getField("id");
            $where['parentid'] = $pid;
        } elseif ($parentid) {  // 上级id
            $where['parentid'] = $parentid;
        }
        if ($regdatetime) {
            list($starttime, $endtime) = explode('|', $regdatetime);
    ...
    public function editChannelAccount()
    {
        if (IS_POST) {
            $userid   = intval(I('post.uid'));
            $isstatus = I('post.isopen') ? I('post.isopen') : 0;
            $res      = M('Member')->where(['id' => $userid])->save(['open_channel_account' => $isstatus]);
            $this->ajaxReturn(['status' => $res]);
        }
    }
