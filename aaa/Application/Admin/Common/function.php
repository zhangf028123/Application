    ...
    if($type == 2){
        foreach($array as $key => $vo){
            if($vo['pid'] == $fid){
                ...
                $vo[$vo['id']] = get_column($array,$type=2,$vo['id'],$level+1);
                $column[] = $vo;
            }
        }
        // 排序
        uasort($column, function($a, $b){
...
function DatabaseBackup(){
    ...
    $file_path = $doc_root.'/sqlbackup';    // 备份路径
    $name = 'backup_'.date('YmdHis').'.sql';
    file_exists($file_path) || mkdirs($file_path, 0777);
    $hostname = C('DB_HOST');
    $hostport = C('DB_PORT');
    $database = C('DB_NAME');
    $user = C('DB_USER');
    ...
    $process = "/usr/bin/mysqldump -h $hostname -u $user -p $password $database > $file_path/$name";
    $er = system($process);
    if ($er !== false){
        return true;
    }
