...
class AutodfController extends Controller
{
    ...
    private function doDf($config)
    {
        ...
        $map['status'] = 0;
        $map['auto_submit_try'] = ['lt',5]; // 自动代付尝试提交次数,失败超过5次的不处理
        $map['df_lock'] = 0;
        ...
        foreach($lists as $k => $v) {
            if($config['auto_df_max_count']>0) {//商户每天自动代付笔数限制
                ...
                if($count>=$config['auto_df_max_count']) {
                    M('Wttklist')->where(['id'=>$v['id']])->save(['last_submit_time'=>time(),'auto_submit_try'=>['exp','auto_submit_try+1'],'df_lock'=>0]); // 自动代付尝试提交次数+1
