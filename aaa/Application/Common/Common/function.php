...
{
    $prefix = 'HX';
    return $prefix.date('YmdHis').substr(implode(NULL, array_map('ord', str_split(substr(uniqid('',true), 7, 17), 1))), 0, 6);
}
...
/// 子账号开关日志
function write_account_switch_log($account_id, $switchtype, $optype, $logstr = null){
    if( !is_array($account_id))
        $account_id = [$account_id];
    foreach($account_id as $aid)
        M('account_switch_log')->add([
            'account_id'=>$aid,
...
function account_switch_type_desc($switchtype){
    $arr = [
        '账号状态',
        '心跳开关',
        '测试状态',
    ...
    return $arr[$switchtype] ?: 'unknoww';
}

function account_switch_log_desc($optype){
    $arr = [
        '管理员关', // 0
        '管理员开', // 1
        '码商关',   // 2
