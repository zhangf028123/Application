...
class WuXiangPayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('WuXiangPay pay url=' . $return['gateway'] . ' data=' . json_encode($data), 'INFOR', true);
        $response = json_decode($response, true);
        Log::record('WuXiangPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('WuXiangPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['status'] == '1') {
            ...
            Log::record('WuXiangPay  $response code is failt ', 'ERR', true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" WuXiangPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，WuXiangPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $data = [
            'fxid' => $_REQUEST["fxid"],
        ...
        $paymoney = $_REQUEST["fxfee"];// 交易金额


        //       判断上游返回的状态是不是正常的，成功的再去检查签名
        if (is_null($response["fxstatus"]) || $response["fxstatus"] != '1') {
            Log::record(' WuXiangPay orderid= ' . $orderid . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
...
//            签名【md5(订单状态+商务号+商户订单号+支付金额+商户秘钥)】
            ...
            if ($result) {
                ...
                Log::record(' WuXiangPay orderid= ' . $orderid . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    }
