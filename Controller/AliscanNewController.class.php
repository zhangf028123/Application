...
/// 拼多多-交易二维码
class AliscanNewController extends PayController
{
    public function Pay($array)
    {
     ...
     //       'amount' => $return["amount"],
        ...
        Log::record('AliscanNew pay url=' . $return['gateway'] . ',data=' . json_encode($data) . ',response=' . $response, 'ERR', true);

        $response = json_decode($response, true);
        $url = 'https://www.kuaizhan.com/common/encode-png?large=true&data=' . $response.['data']['qrcode'];
        if ($response['code'] != '200' || $contentType == 'json'){
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，aliscan New notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($sign == $response['sign'] && $response['callbacks'] == 'CODE_SUCCESS') {
            try{
                ...
                if(!$o){
                    Log::record('支付宝扫码new回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("支付宝扫码new回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $diff1 = $response['amount_true'] - $pay_amount;
                if($diff1 <= -1 || $diff1 >= 1 ){ // 允许误差一块钱
                    Log::record("支付宝扫码new回调失败11,金额不等：{$response['amount_true'] } != {$pay_amount},".json_encode($response),'ERR',true);
                ...
                if( $old_order && $old_order['pay_orderid'] != $response["out_trade_no"]){
                    Log::record("支付宝扫码new回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('小额话费回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
