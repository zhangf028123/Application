...
class DDPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            'pay_bankcode'=> $return['appid'],//对接编码在商户中心查询
            'pay_notifyurl' =>$return['notifyurl'],
            'pay_callbackurl'=>$return['notifyurl'].'/callback',
            'pay_amount' => sprintf("%.2f", $return["amount"]),
        ];
        $sign = $this->getSign($return["signkey"], $data);
        $data['pay_md5sign'] = $sign;
        $data['pay_returntype'] ='json';
//        pay_returntype  默认表单跳转,需要json返回,该值填写json
//        pay_type   支付宝:alipay,微信:wechat
        //程序获取参数
        $response = HttpClient::post($return['gateway'], $data);
        Log::record('DDPay pay url=' . $return['gateway'] . ' response=' . $response , 'INFOR', true);

        $cost_time = $this->msectime() - $start_time;
        $response = json_decode($response, true);
        Log::record('DDPay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('DDPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1' ) {
            ...
            Log::record('DDPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
    }
...
//        $response = file_get_contents("php://input");
        $clientip = $_SERVER['REMOTE_ADDR'];
        Log::record(" DDPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，DDPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
//        $response = json_decode($response,true);
        $data = [
            'memberid' => $response["memberid"],
            'pay_orderid' => $response['pay_orderid'],//
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["returncode"])||$response["returncode"] != '00' ){
            Log::record(' DDPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' DDPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
...
//        $string_a= substr($string_a, 0, -1);
        ...
        Log::record('DDPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
