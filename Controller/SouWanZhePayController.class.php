...
class SouWanZhePayController extends PayController
{
 ...
 //异步通知
    public function notifyurl()
    {
        ...
        Log::record(" SouWanZhePay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，SouWanZhePay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        if(is_null($response["returncode"])||$response["returncode"]!='00' ){
            Log::record(' SouWanZhePay orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' SouWanZhePay orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    }
