...
class HaiMoPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data['pay_type'] = 'wechat';//产品名字   支付宝:alipay,微信:wechat
        $data['pay_returntype']='json';
        $response = HttpClient::post($return['gateway'], $data);
        $cost_time = $this->msectime() - $start_time;
        Log::record('HaiMoPay pay url=' . $return['gateway'] . ' data=' . json_encode($data), 'INFOR', true);
        $response = json_decode($response, true);
        Log::record('HaiMoPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('HaiMoPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1') {
            ...
            Log::record('HaiMoPay  $response code is failt ', 'ERR', true);
 ...
 //异步通知
    public function notifyurl()
    {
        ...
        Log::record(" HaiMoPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，HaiMoPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        $orderid = $_REQUEST["pay_orderid"];//自己的订单号
        $upstream_order_id = $_REQUEST["transaction_id"];// 上游的订单id,
        $paymoney = $_REQUEST["amount"];// 交易金额


        //       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["returncode"])||$response["returncode"]!='00' ){
            Log::record(' HaiMoPay orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' HaiMoPay orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    }
