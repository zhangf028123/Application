...
class MenTuiPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data['pay_returntype'] = 'json';//返回json
        //程序获取参数
        $response = HttpClient::post($return['gateway'], $data);
        $cost_time = $this->msectime() - $start_time;
        $response = json_decode($response, true);
        Log::record('MenTuiPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('MenTuiPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1' ) {
            ...
            Log::record('MenTuiPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
    }
...
//        $response = file_get_contents("php://input");
        ...
        Log::record(" MenTuiPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，MenTuiPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $data = [
            ...
            'transaction_id'=> $response["transaction_id"],//上游订单id
            'datetime' => $response['datetime'],//订单时间
            'returncode' => $response['returncode'],//00表示成功，其它表示失败
            'sign' => $response['sign'],
        ];

//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["returncode"])||$response["returncode"] != '00' ){
            Log::record(' MenTuiPay回调 orderid= ' . $response['orderid'] . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' MenTuiPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
