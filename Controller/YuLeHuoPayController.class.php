...
class YuLeHuoPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data["sign"] = $sign;
        //程序获取参数
        Log::record('YuLeHuoPay pay url=' . $return['gateway'] . ' data= ' . json_encode($data) , 'INFOR', true);
...
//        $response = file_get_contents("php://input");
        ...
        Log::record(" YuLeHuoPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，YuLeHuoPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["code"])||$response["code"]!='00' ){
            Log::record(' YuLeHuoPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' YuLeHuoPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
...
//        $string_a = rtrim($string_a, '&');
        ...
        Log::record('YuLeHuoPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
