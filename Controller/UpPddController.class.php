...
class UpPddController extends PayController
{
    public function Pay($array){
        ...
        Log::record('UpPdd pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
        ...
        if (parent::isMobile()){ // 手机直接跳转
            ...
            \Think\Log::record('Uppdd mobile'.json_encode($response), 'ERR', true);
            header("location: {$response['url']}");
        }
