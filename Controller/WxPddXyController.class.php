...
class WxPddXyController extends PayController
{
    public function Pay($array){
        ...
        Log::record('WxPddXy pay url='.$return['gateway'].',data='.json_encode($params).',response='.$res,'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" WxPddXy \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，WxPddXy notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if (isset($_POST['sign']) && (2 === intval($_POST['status']) || 10 === intval($_POST['status']) || 11 === intval($_POST['status']))) {
            ...
            if (!$res) {
                Log::record('WxPddXy回调失败,验签失败：'.$res,'ERR',true);
            ...
            try{
                ...
                if(!$o){
                    Log::record('WxPddXy回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("WxPddXy回调失败,金额不等：{$response['money'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $old_order = $Order->where(['upstream_order'=>$response['orderSn']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["orderSn"]){
                    Log::record("WxPddXy回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('WxPddXy回调回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        }
