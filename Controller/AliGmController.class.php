...
class AliGmController extends PayController
{
    public function Pay($array){
        ...
        Log::record(' AliGm pay url='.$return['gateway'].',data='.json_encode($data).',response='.$response,'ERR',true);
        echo $response;
    ...
    public function notifyurl()
    {
        ...
        Log::record(" AliGm \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，AliGm notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result) {
            try{
                ...
                if(!$o){
                    Log::record('国美支付宝回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("国美支付宝回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $old_order = $Order->where(['upstream_order'=>$response['transaction_id']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["orderid"]){
                    Log::record("国美支付宝回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('国美支付宝回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } else {
            Log::record('AliGm error:check sign Fail!','ERR',true);
