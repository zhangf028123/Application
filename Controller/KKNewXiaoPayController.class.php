...
class KKNewXiaoPayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('KKNewXiaoPay pay url=' . $return['gateway'] . ' data=' . json_encode($data), 'INFOR', true);
        $response = json_decode($response, true);
        Log::record('KKNewXiaoPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('KKNewXiaoPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '200') {
            ...
            Log::record('KKNewXiaoPay  $response code is failt ', 'ERR', true);
            exit();
        }
        echo $response;
    }
 //异步通知
    public function notifyurl()
    {
        ...
        Log::record(" KKNewXiaoPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，KKNewXiaoPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        if ($result) {
            ...
            Log::record('KKNewXiaoPay error:check sign Fail!', 'ERR', true);
