...
class PddALController extends PayController
{
    public function Pay($array){
        ...
        Log::record(' PddAl pay url='.$return['gateway'].',data='.json_encode($data).',response='.json_encode($res).',cost time='.$cost_time,'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，PddAl notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        $publiKey = getKey($response["api_order_sn"]); // 密钥
        Log::record("PddAl notifyurl".json_encode($_REQUEST),'ERR',true);
        $data = [
        ...
        if ($data['sign'] == $sign &&  $data['callbacks'] == 'CODE_SUCCESS') {
            try{
                ...
                if(!$o){
                    Log::record('pddal回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("pddal回调失败,金额不等：{$response['total'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }

                $old_order = $Order->where(['upstream_order'=>$response['order_sn']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["order_sn"]){
                    Log::record("pddal回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('pddal回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } else {
