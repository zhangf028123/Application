...
class ZfbXiYuController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        Log::record('ZfbXiYu pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        Log::record('支付宝原生小额订单失败：'.$response.'--'.json_decode($response).'--'.json_encode($response),'ERR',true);
        $response = json_decode($response, true);

        $clientip = $_SERVER['REMOTE_ADDR'];
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，ZfbXiYu notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
