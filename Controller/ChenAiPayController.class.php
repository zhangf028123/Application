...
class ChenAiPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            "pay_callbackurl" => $clientip,
            "pay_amount" => $return["amount"],
        ];
        $sign = $this->getSign($return["signkey"], $data);
        $data["pay_md5sign"] = $sign;
        $data["pay_productname"] = '钻石';

        $this->setHtml($return['gateway'], $data);
        die; // 上游跳转
        //程序获取参数
//        $response = HttpClient::post($return['gateway'], $data);
//        Log::record('ChenAiPay pay url=' . $return['gateway'] . ' response=' . $response , 'INFOR', true);
//
//        $cost_time = $this->msectime() - $start_time;
//        $response = json_decode($response, true);
//        Log::record('ChenAiPay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
//        if (empty($response)) {
//            Log::record('ChenAiPay  $response is empty ', 'ERR', true);
//            exit();
//        }
//        if ($response['code'] == '0' ) {
...
//            exit();
//        } else {
//            Log::record('ChenAiPay  orderid= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
//            exit();
//        }
    }
...
//        $response = file_get_contents("php://input");
        $clientip = $_SERVER['REMOTE_ADDR'];
        Log::record(" ChenAiPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，ChenAiPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["returncode"])||$response["returncode"]!='00' ){
            Log::record(' ChenAiPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' ChenAiPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
...
//        $string_a = rtrim($string_a, '&');
        ...
        Log::record('ChenAiPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
