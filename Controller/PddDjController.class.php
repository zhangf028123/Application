...
class PddDjController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('Pdddj pay url='.$return['gateway'].',data='.json_encode($data).',response='.$response,'ERR',true);

        $response = json_decode($response, true);
        if ($response['response']['code'] == '10000' ) {
            $pid = $array['pid'];
            if ($contentType == 'json' && $pid == 946){
                //支付宝
                $return = [
                    'result' => 'ok',
                    'orderStr' => $response['response']['alisdk'],
                ];
    ...
    public function notifyurl()
    {
        ...
        Log::record('pdd(dj)订单失败：'.$response.'--'.json_decode($response).'--'.json_encode($response),'ERR',true);
        $response = json_decode($response, true);

        $clientip = $_SERVER['REMOTE_ADDR'];
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，pdddj notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($sign == $response['sign'] && $response['status'] == '5') {
            try{
                ...
                if(!$o){
                    Log::record('pdd(dj)回调失败,找不到订单：'.$response,'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("pdd(dj)回调失败,金额不等：{$response['total_fee'] } != {$pay_amount},".$response,'ERR',true);
                ...
                if( $old_order && $old_order['pay_orderid'] != $response['out_trade_no']){
                    Log::record("pdd(dj)回调失败,重复流水号  ：".$response.'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('pdd(dj)回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } elseif ($sign == $response['sign']) {
            //未支付。。。
            Log::record('pdd(dj)订单失败：'.$response,'ERR',true);
            exit("error:not pay");
        }
        else {
            Log::record('pdd(dj)订单失败：'.$sign.'---'.$response['sign'],'ERR',true);
