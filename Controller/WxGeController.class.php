...
class WxGeController extends PayController
{
    public function Pay($array){
        ...
        //$this->setHtml($return['gateway'], $data);die; // 上游跳转

        $response = HttpClient::post($return['gateway'], $data);    //
        $cost_time = $this->msectime() - $start_time;
        Log::record('WxGe pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
        $response = json_decode($response, true);
        
        if($response['status'] == 1){
            header("location: {$response['h5_url']}");
        }
    ...
    public function notifyurl()
    {
        ...
        Log::record(" WxGe \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，WxGe notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
