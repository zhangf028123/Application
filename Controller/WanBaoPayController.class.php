...
class WanBaoPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            'merchant_code' => $return['mch_id'],//商户号,
            'order_id' => $return['orderid'], //商户订单号
            'merchant_time'=>date("Y-m-d H:i:s"),//时间
            'paytype' => $return['appid'],//对接编码在商户中心查询
            'subject'=>'零售',
            'body'=>'零售',
        ...
        Log::record('WanBaoPay pay url=' . $return['gateway'] . ' response=' . $response , 'INFOR', true);

        $cost_time = $this->msectime() - $start_time;
        $response = json_decode($response, true);
        Log::record('WanBaoPay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('WanBaoPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '0' ) {
            ...
            Log::record('WanBaoPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
    }
...
//        $response = $_REQUEST;/
        ...
        Log::record(" WanBaoPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，WanBaoPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $response = json_decode($response,true);
        $data = [
            ...
            'order_id'=> $response["order_id"],//我们自己的订单号
            'pay_no' => $response['pay_no'],//上游的订单id
            'amount' => $response['amount'],
            'resp_code'=>$response['resp_code'],//支付状态 00成功 其它失败
            'sign' => $response['sign'],
        ];
        $orderid = $response["order_id"];//自己的订单号
        $upstream_order_id = $response["pay_no"];// 上游的订单id,
        $paymoney = $response["amount"];// 交易金额

//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["code"])||$response["code"]!='0'||is_null($response["resp_code"])||$response["resp_code"] != '00' ){
            Log::record(' WanBaoPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' WanBaoPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    ...
    private function getSign($secret, $data)
    {
        ...
        Log::record('WanBaoPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
