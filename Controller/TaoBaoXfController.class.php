...
class TaoBaoXfController extends PayController
{
    public function Pay($array){
        ...
        */
        ...
        Log::record('TaoBaoXf pay url='.$return['gateway'].',data='.json_encode($data).',response='.$response,'ERR',true);
        echo $response;
    ...
    public function notifyurl()
    {
        ...
        Log::record(" TaoBaoXf \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，TaoBaoXf notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result) {
            try{
                ...
                if(!$o){
                    Log::record('淘宝红包xf回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("淘宝红包xf回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
                ...
                Log::record('淘宝红包xf回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } else {
            Log::record('TaoBaoXf error:check sign Fail!','ERR',true);
