...
class AliPersonController extends PayController
{
    public function Pay($array){
       ...
       // $data['content_type']       = 'json';
        ...
        Log::record(' AliPerson pay url='.$return['gateway'].',data='.json_encode($data).',response='.$response,'ERR',true);
        echo $response;
    ...
    public function notifyurl()
    {
        ...
        Log::record(" AliPerson \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，AliPerson notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result) {
            try{
                ...
                if(!$o){
                    Log::record('支付宝个码回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("支付宝个码回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
                ...
                Log::record('支付宝个码回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } else {
            Log::record('AliPerson error:check sign Fail!','ERR',true);
