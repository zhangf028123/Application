...
class CanWeiPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data['sign_type']="MD5";
        //程序获取参数
        $response = $this->httpGet($return['gateway'], $data);
        Log::record('CanWeiPay pay url=' . $return['gateway'] . ' response=' . $response , 'INFOR', true);

        $cost_time = $this->msectime() - $start_time;
        $response = json_decode($response, true);
        Log::record('CanWeiPay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('CanWeiPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1' ) {
            ...
            Log::record('CanWeiPay  nonce_str= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
    }
    /**
    ...
    private function httpGet($url, $postData='') {
        Log::record('CanWeiPay  $postDat=  '.http_build_query($postData), 'ERR', true);
        $restUrl=$url .'?'.http_build_query($postData);
        Log::record('CanWeiPay  $restUrl= ' . $restUrl, '  ERR', true);
...
//        $response = file_get_contents("php://input");
        ...
        Log::record(" CanWeiPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，CanWeiPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        $data = [
            'id' => $response["id"],
            'name' => $response['name'],
            'trade_status' => $response["trade_status"],//交易状态：TRADE_SUCCESS / TRADE_FAIL
            'out_trade_no'=> $response["out_trade_no"],//我们自己的订单号
            'trade_no' => $response['trade_no'],//上游的订单id
            'money' => $response['money'],
//            'sign_type'=>$response['sign_type'],//支付状态 00成功 其它失败
            'sign' => $response['sign'],
        ];
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["trade_status"])||$response["trade_status"]!='TRADE_SUCCESS' ){
            Log::record(' CanWeiPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' CanWeiPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    ...
    private function getSign($secret, $data)
    {
        ...
        Log::record('CanWeiPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
