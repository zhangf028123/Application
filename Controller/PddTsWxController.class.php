...
class PddTsWxController extends PayController
{
    public function Pay($array){
        ...
        Log::record('PddTsWx pay url='.$return['gateway'].',data='.json_encode($data).',response='.$response,'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" PddTsWx \$response=".json_encode($response),'ERR',true);
        $response = json_decode($response, true);

        $clientip = $_SERVER['REMOTE_ADDR'];
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，PddTsWx notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result == $sign && ($data['pay_status'] == '1' || $data['pay_status'] == '3')) {
            try{
                ...
                if(!$o){
                    Log::record('拼多多Ts(wx)回调失败,找不到订单：'.json_encode($response),'ERR',true);
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("拼多多Ts(wx)回调失败,金额不等：{$response['order_amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
                    exit('error: amount error!');
                }
                $old_order = $Order->where(['upstream_order'=>$response['order_sn']])->find();
                if( $old_order && $old_order['pay_orderid'] != $response["outorderno"]){
                    Log::record("拼多多Ts(wx)回调失败,重复流水号  ：".json_encode($response).'旧订单号'.$old_order['pay_orderid'],'ERR',true);
                    //die("not ok2");
                }
                ...
                Log::record('拼多多Ts(wx)回调失败,发生异常：'.$e->getMessage(),'ERR',true);
                exit("Exception");
            }
        } else {
            Log::record('PddTsWx error:check sign Fail!','ERR',true);
