...
class MCTPayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('MCTPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('MCTPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['retCode'] == '0' ) {
            ...
            Log::record('MCTPay  orderid  = ' . $return['orderid'] . '.pay failt ', 'ERR', true);
...
// 异步通知
    public function notifyurl()
    {
        $response = $_REQUEST;
        $clientip = $_SERVER['REMOTE_ADDR'];
        Log::record(" MCTPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，MCTPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        $data = [
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if (!is_null($response["status"]) &&($response["status"] == '2'||$response["status"] == '3')) {
            ...
            if ($result) {
                ...
                $upstream_order_id = $response["channelOrderNo"];// 上游的订单id,
                ...
                Log::record(' MCTPay回调 orderid= ' . $response['out_trade_no'] . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }

        } else {
            Log::record('MCTPay回调 orderid= ' . $response['mchOrderNo'] . '  状态失败 $response' . json_encode($response), 'ERR', true);
            exit("fail");
        }
