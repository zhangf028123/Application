...
class PuKePayController extends PayController
{
    public function Pay($array)
    {
        ...
        Log::record('PuKePay pay url=' . $return['gateway'] . ' data=' . json_encode($data), 'INFOR', true);
        $response = json_decode($response, true);
        Log::record('PuKePay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('PuKePay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1') {
            ...
            Log::record('PuKePay  $response code is failt ', 'ERR', true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" PuKePay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，PuKePay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        if (is_null($response["returncode"]) || $response["returncode"] != '00') {
            Log::record(' PuKePay orderid= ' . $orderid . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' PuKePay orderid= ' . $orderid . '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    }
