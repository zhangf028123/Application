...
class YuLeHuoUrlPayController extends PayController
{
    public function Pay($array)
    {
        ...
        $data = [
            ...
            "paytype" =>'WXPAY',//	支付方式	否	是	参数：ALIPAY、WXPAY，默认支付宝（ALIPAY）
        ...
        $data["sign"] = $sign;
        //程序获取参数
        Log::record('YuLeHuoUrlPay pay url=' . $return['gateway'] . ' data= ' . json_encode($data) , 'INFOR', true);
        $response = HttpClient::post($return['gateway'], $data);

        Log::record('YuLeHuoUrlPay pay url=' . $return['gateway'] . ' response=' . $response , 'INFOR', true);
//
        $cost_time = $this->msectime() - $start_time;
        $response = json_decode($response, true);
        Log::record('YuLeHuoUrlPay pay url=' . $return['gateway'] . ' response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('YuLeHuoUrlPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['pay_url'] != '' ) {
            ...
            Log::record('YuLeHuoUrlPay  orderid= ' .  $return['orderid'] . '.is failt ', 'ERR', true);
            exit();
        }
    }
...
//        $response = file_get_contents("php://input");
        ...
        Log::record(" YuLeHuoUrlPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，YuLeHuoUrlPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
...
//       判断上游返回的状态是不是正常的，成功的再去检查签名
        if(is_null($response["code"])||$response["code"]!='00' ){
            Log::record(' YuLeHuoUrlPay回调 orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' YuLeHuoUrlPay回调 orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
...
//        $string_a = rtrim($string_a, '&');
        ...
        Log::record('YuLeHuoUrlPay createToSignStr ===== $string_sign_temp  ：' . $string_sign_temp, 'ERR', true);
        //签名步骤三：MD5加密
