...
class UpPddSdkController extends PayController
{
    public function Pay($array){
        ...
        Log::record('UpPddSdk pay url='.$return['gateway'].'data='.json_encode($data).'response='.$response."cost time={$cost_time}ms",'ERR',true);
    ...
    public function notifyurl()
    {
        ...
        Log::record(" UpPddSdk \$response=".json_encode($response),'ERR',true);
        $ip = getIP(); // 可能是伪造的
        if($clientip != $ip){
            Log::record("伪造的ip，UpPddSdk notifyurl， clientip={$clientip}, getIP = $ip",'ERR',true);
            die("not ok1");
        }
        ...
        if ($result) {
            try{
                ...
                if($diff <= -1 || $diff >= 1 ){ // 允许误差一块钱
                    Log::record("上游sdk回调失败,金额不等：{$response['amount'] } != {$pay_amount},".json_encode($response),'ERR',true);
            ...
            Log::record('UpPddSdk error:check sign Fail!','ERR',true);
