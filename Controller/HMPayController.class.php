...
class HMPayController extends PayController
{
...
//        $data['format']='json';
        ...
        Log::record('HMPay pay url=' . $return['gateway'] . ' data=' . json_encode($data), 'INFOR', true);
        $response = json_decode($response, true);
        Log::record('HMPay pay url=' . $return['gateway'] . 'response=' . json_encode($response) . " cost time={$cost_time}ms", 'INFOR', true);
        if (empty($response)) {
            Log::record('HMPay  $response is empty ', 'ERR', true);
            exit();
        }
        if ($response['code'] == '1') {
            ...
            Log::record('HMPay  $response code is failt ', 'ERR', true);
 ...
 //异步通知
    public function notifyurl()
    {
        ...
        Log::record(" HMPay notifyurl \$response=" . json_encode($response), 'ERR', true);
        $ip = getIP(); // 可能是伪造的
        if ($clientip != $ip) {
            Log::record("伪造的ip，HMPay notifyurl， clientip={$clientip}, getIP = $ip", 'ERR', true);
            die("not ok1");
        }
        ...
        if(is_null($response["returncode"])||$response["returncode"]!='00' ){
            Log::record(' HMPay orderid= ' .$orderid  . '  订单状态失败 $response=' . json_encode($response), 'ERR', true);
            exit("fail");
        } else {
            ...
            if ($result) {
                ...
                Log::record(' HMPay orderid= ' . $orderid. '  error:check sign Fail! $response' . json_encode($response), 'ERR', true);
                exit('fail');
            }
        }
    }
